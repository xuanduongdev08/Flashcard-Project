...
<div class="max-w-3xl mx-auto px-4 animate-fade-in pb-20">
    <div class="mb-10 flex flex-col items-center text-center">
        ...
        <h1 class="text-3xl font-black text-slate-900 dark:text-white mb-2 leading-snug tracking-tighter">Thêm bộ sưu tập mới</h1>
    ...
    <form action="{{ route('decks.store') }}" method="POST"
          class="bg-white dark:bg-slate-900
                 ...
                 p-8 md:p-12 rounded-3xl space-y-8">
        @csrf
                ...
                <div class="relative">
                    <select name="language_id" required
                            class="w-full
                                   ...
                                   focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500
                                   outline-none transition-all
                                   appearance-none cursor-pointer font-bold">
                    ...
                    <div class="pointer-events-none absolute inset-y-0 right-5 flex items-center">
                        <svg class="w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M19 9l-7 7-7-7"/></svg>
                    </div>
                </div>
        ...
        <div class="flex flex-col sm:flex-row items-center gap-4 pt-4">
            <button type="submit"
                    class="w-full sm:flex-1 py-5 btn-gradient
                           font-black rounded-2xl
                           shadow-xl shadow-indigo-600/20
                           transition transform active:scale-95 leading-none">
                Tạo bộ thẻ mới ✨
            </button>
            <a href="{{ route('dashboard') }}"
               class="w-full sm:w-auto px-10 py-5
                      bg-slate-100 dark:bg-slate-800
                      hover:bg-slate-200 dark:hover:bg-slate-700
                      text-slate-600 dark:text-slate-400
                      hover:text-slate-900 dark:hover:text-white
                      font-bold rounded-2xl transition text-center
