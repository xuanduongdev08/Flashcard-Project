...
<div class="px-4 animate-fade-in pb-20">
    <div class="mb-10">
        <h1 class="text-3xl font-black text-slate-900 dark:text-white mb-2 leading-snug tracking-tighter">Thêm thẻ mới</h1>
        <p class="text-slate-500 font-medium">Đang thêm vào bộ thẻ: <strong class="text-slate-900 dark:text-white">{{ $deck->title }}</strong></p>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        {{-- Form Side --}}
        <form action="{{ route('decks.cards.store', $deck) }}" method="POST"
            ...
            <div class="flex flex-col sm:flex-row items-center gap-4 pt-4 border-t border-slate-100 dark:border-slate-800/50">
                ...
                <a href="{{ route('decks.show', $deck) }}" class="w-full sm:w-auto px-10 py-5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-bold rounded-2xl transition text-center border border-slate-200 dark:border-transparent">
                    Bỏ qua
                </a>
            </div>
        </form>
            ...
            <div class="flip-card aspect-[4/2.5] max-w-lg mx-auto lg:mx-0 w-full group cursor-pointer" onclick="this.querySelector('.flip-card-inner').classList.toggle('flipped')">
                <div class="flip-card-inner relative w-full h-full shadow-2xl transition-all duration-700 rounded-3xl">
                    ...
                    <div class="flip-card-front absolute inset-0 glass border-2 rounded-3xl flex flex-col items-center justify-center p-12 text-center"
                        ...
                        <p class="text-3xl font-black text-slate-900 dark:text-white leading-tight" id="frontPreview">Nội dung sẽ hiển thị ở đây...</p>
                    ...
                    <div class="flip-card-back absolute inset-0 bg-white dark:bg-slate-900 border-2 rounded-3xl flex flex-col items-center justify-center p-12 text-center"
                        ...
                        <p class="text-lg text-slate-800 dark:text-slate-300 leading-relaxed font-medium" id="backPreview">Đáp án hoặc lời giải thích sẽ nằm ở đây.</p>
                    </div>
                </div>
            </div>
