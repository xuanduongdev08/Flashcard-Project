...
<div class="px-4 animate-fade-in pb-20">
    <div class="mb-10">
        <h1 class="text-3xl font-black text-white mb-2 leading-snug tracking-tighter">Cập nhật nội dung thẻ</h1>
        <p class="text-slate-500 font-medium">Bộ thẻ: <strong class="text-white">{{ $card->deck->title }}</strong></p>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        {{-- Form Side --}}
        <form action="{{ route('decks.cards.update', [$card->deck, $card]) }}" method="POST" class="bg-slate-900 shadow-2xl border border-slate-800 p-8 md:p-10 rounded-3xl space-y-8">
            @csrf @method('PUT')
            ...
            <div class="flex flex-col sm:flex-row items-center gap-4 pt-4 border-t border-slate-800/50">
                ...
                <a href="{{ route('decks.show', $card->deck) }}" class="w-full sm:w-auto px-10 py-5 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white font-bold rounded-2xl transition text-center">
                    Hủy sửa
                </a>
            </div>
        </form>
            ...
            <div class="flip-card aspect-[4/2.5] max-w-lg mx-auto lg:mx-0 w-full group cursor-pointer" onclick="this.querySelector('.flip-card-inner').classList.toggle('flipped')">
                <div class="flip-card-inner relative w-full h-full shadow-2xl transition-all duration-700 rounded-3xl">
                    ...
                    <div class="flip-card-front absolute inset-0 glass border-2 border-slate-800 rounded-3xl flex flex-col items-center justify-center p-12 text-center"
                        ...
                        <p class="text-3xl font-black text-white leading-tight" id="frontPreview">{{ $card->front }}</p>
                    ...
                    <div class="flip-card-back absolute inset-0 bg-slate-900 border-2 border-indigo-500/50 rounded-3xl flex flex-col items-center justify-center p-12 text-center"
                        ...
                        <p class="text-lg text-slate-300 leading-relaxed font-medium" id="backPreview">{{ $card->back }}</p>
                    </div>
                </div>
            </div>
